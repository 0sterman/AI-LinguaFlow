...
def test_find_existing_install_root_prefers_registered_linguaflow_root(monkeypatch, tmp_path) -> None:
    registered_root = tmp_path / "registered" / "LinguaFlow AI"
    target_root = tmp_path / "target" / "LinguaFlow AI"
    registered_root.mkdir(parents=True)
    target_root.mkdir(parents=True)
    (registered_root / "LinguaFlow AI.exe").write_bytes(b"exe")
...
def test_find_existing_install_root_uses_target_linguaflow_root(monkeypatch, tmp_path) -> None:
    target_root = tmp_path / "LinguaFlow AI"
    target_root.mkdir()
    (target_root / "LinguaFlow AI Uninstall.exe").write_bytes(b"exe")
